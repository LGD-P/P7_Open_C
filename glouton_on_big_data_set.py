...
df = Reader.csv_reader("data/dataset2_Python+P7.csv")
dfAction = Reader.creat_list_from_culumn(df, "name")
dfPrice = Reader.creat_list_from_culumn(df, 'price')
...
def algo_glouton(max_investment, list_of_data):
    ...
    for element in range(len(list_of_data)):
        if list_of_data[element]["Price"] > 0.0 and spent + list_of_data[element]["Price"] <= max_investment:
            ...
            print(list_of_data[element]["Price"])
